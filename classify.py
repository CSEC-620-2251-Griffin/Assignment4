...
def load_iot_dataset(root, min_samples=20, max_samples=1000):
    """
    Loads IoT dataset from a multi-layer folder structure.
    Returns:
        X: merged feature matrix
        y: encoded labels
        le: LabelEncoder
    """
    # Use the multi-feature loader
    X_misc, X_p, X_d, X_c, Y_str = load_data(
        root,
        min_samples=min_samples,
        max_samples=max_samples
    )

    # Encode device labels (MAC addresses)
    le = LabelEncoder()
    y = le.fit_transform(Y_str)

    # Merge all feature components
    X = np.hstack([X_misc, X_p, X_d, X_c]).astype(float)
